function [x, fval, output, options] = optimize_NelderMead(fun,xmin,xmax,options)
% Globalized and bounded Nelder-Mead as published by Luersen and Le Riche
% 
...
%--- INIT ---
    ...
    if nargin < 4
        ...
        options.maxIter=1000; % maximum iterations per restart
        options.alpha = 1; % reflection coeff
        options.beta = 0.5; % contraction coeff
        options.gamma = 2; % expansion coeff
        options.epsilon = 1e-9; % T2 convergence test coefficient
